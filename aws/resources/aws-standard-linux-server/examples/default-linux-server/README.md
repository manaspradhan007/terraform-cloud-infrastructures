# Standard Linux Server with Default Settings

This example creates a Standard Linux Server with all the default settings - taking only the required values from the consumer.

## Usage
This example can be executed by executing:
```bash
$ terraform init
$ terraform plan
$ terraform apply
```

## Destruction
This example creates resources which may incur cost from the cloud provider. Resources, when not needed, can be destroyed by executing:
```bash
$ terraform destroy
```