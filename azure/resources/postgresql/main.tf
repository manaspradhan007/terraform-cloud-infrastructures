locals {
  server_name = azurerm_postgresql_flexible_server.postgresqlserver.0.name
}
...
resource "azurerm_resource_group" "db_rg" {
  count    = var.create_postgresql_resource ? 1 : 0
...
resource "azurerm_postgresql_flexible_server" "postgresqlserver" {
  ...
  location               = var.location
  name                   = "<name>-${var.project}-${var.suffix}-${var.environment_short}"
